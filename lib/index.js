...
function distance(graph, v1, v2) {
   ...
   */
  let x1 = graph.nodeCoord[v1][0];
  let x2 = graph.nodeCoord[v2][0];
  let y1 = graph.nodeCoord[v1][1];
  let y2 = graph.nodeCoord[v2][1];
...
function deleteEdge(graph) {
  ...
  newGraph.nodeCoord = graph.nodeCoord;
...
function summary(graphList) {
  ...
  for (let g = 0; g < graphList; g++) {
    ...
    let currMax = 0;
    for (let n = 0; n < g.nodeCoord.length; n++) {
      let dist = jsnx.ijkstra(g, 0, n);
      if (dist > currMax) {
        currMax = dist;
      }
    }
